      ...
      success: function (xmlResponse) {
        ...
        $input.addEventListener('input', function () {
          ...
          datas.forEach(function (data) {
            ...
            var dataContent = data.content.trim().replace(
              /<[^>]+>/g, '').toLowerCase()
            ...
            if (dataTitle !== '' && dataContent !== '') {
              keywords.forEach(function (keyword, i) {
                ...
                indexContent = dataContent.indexOf(
                  keyword)
            ...
            if (isMatch) {
              str +=
                ...
                dataUrl + '" class="search-result-title">' +
                dataTitle + '</a>' + '</div>'
              count += 1
          ...
          if (count === 0) {
            str += '<div id="local-search__hits-empty">' +
              GLOBAL_CONFIG.localSearch.languages.hits_empty
              .replace(/\$\{query}/, this.value.trim()) +
              '</div>'
          }
